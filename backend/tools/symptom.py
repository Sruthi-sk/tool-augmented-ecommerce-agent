...
def _match_symptom(appliance_type: str, symptom: str) -> tuple[dict | None, str]:
    """Fuzzy-match a symptom string against the DB. Returns (entry, matched_key)."""
    appliance_symptoms = SYMPTOM_DB.get(appliance_type.lower(), {})
    symptom_lower = symptom.lower()

    # Exact match
    if symptom_lower in appliance_symptoms:
        return appliance_symptoms[symptom_lower], symptom_lower

    # Substring match
    for key, entry in appliance_symptoms.items():
...
def register_symptom_tool(
    ...
    async def diagnose_symptom(
        ...
        entry, matched_key = _match_symptom(appliance_type, symptom)
